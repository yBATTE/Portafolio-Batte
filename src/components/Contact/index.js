...
const Contact = () => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    emailjs.sendForm('LucasBattelini', 'template_p15k6vo', form.current, 'NwzflZnWBVMc9V0dn').then(
      (result) => {
        form.current.reset();
        Toastify({
          ...
          gravity: 'top',
          position: 'right',
          backgroundColor: 'linear-gradient(225deg, hsla(271, 100%, 50%, 1) 0%, hsla(294, 100%, 50%, 1) 100%)',
        }).showToast();
      },
      (error) => {
